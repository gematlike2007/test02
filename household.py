...
def main():
    ...
    with st.sidebar:
        ...
        with st.form(key="settings_form", clear_on_submit=False):
            new_base_day = st.number_input(
                "월 기준일 (1~31)",
                min_value=1,
                max_value=31,
                value=account.base_day,
                step=1,
                help="한 달의 시작 기준일을 설정합니다.",
                key="input_base_day"
            )

            new_base_money = st.number_input(
                "월 기준 금액 (원)",
                min_value=0,
                value=account.base_money,
                step=10000,
                help="기준일에 자동으로 추가될 금액입니다.",
                key="input_base_money"
            )

            new_nestegg = st.number_input(
                "비상금 (원)",
                min_value=0,
                value=account.nestegg,
                step=10000,
        ...
        if settings_submit:
            ...
            if new_base_day != account.base_day:
                account.base_day = new_base_day
                settings_changed = True
                base_day_changed = True

            if new_base_money != account.base_money:
                account.base_money = new_base_money
                settings_changed = True
                base_money_changed = True

            if new_nestegg != account.nestegg:
                account.nestegg = new_nestegg
                settings_changed = True
            ...
            if settings_changed:
                ...
                st.success(
                    f"✅ 설정이 저장되었습니다!\n- 기준일: {new_base_day}일\n- 기준 금액: {new_base_money:,}원\n- 비상금: {new_nestegg:,}원")

                if base_day_changed or base_money_changed:
                    st.info(f"💡 기준일({new_base_day}일)에 {new_base_money:,}원이 자동으로 추가되었습니다.")
